...
import { apiData, apiError, isMutationAllowed, resolvePlaceId } from '@/backend/http';
import { mapShortRow, shortRowSelect } from '@/backend/shorts';
...
export async function POST(request: NextRequest) {
  ...
  return apiData(mapShortRow(data as unknown as Record<string, unknown>), {
    status: 201,
...
export async function PATCH(request: NextRequest) {
  ...
  const { data, error } = await context.db
    .from('shorts')
    .update(adminShortUpdateColumns(parsed.value, placeId ?? undefined))
    .eq('id', parsed.value.shortId)
    .select(shortRowSelect)
    .maybeSingle();
  if (error) return apiError('SHORT_UPDATE_FAILED', error.message, 500);
  if (!data) return apiError('SHORT_NOT_FOUND', '수정할 쇼츠를 찾을 수 없습니다.', 404);
